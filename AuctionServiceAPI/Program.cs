...
try
{
    ...
    {
        logger.Error(ex, "Could not fetch secrets from Vault after retries");
        return;
    }
    ...
    TimeSpan? delayBetweenRetries = null)
{
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++)
    {
        ...
        {
            Console.WriteLine(
                $"[Vault] Attempt {attempt} failed: {ex.Message}. Waiting {delayBetweenRetries.Value.TotalSeconds} seconds before retry...");
